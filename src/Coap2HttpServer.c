...
get_ongoing_proxy_session(coap_session_t *session,
    ...
    proxy_list_t *new_proxy_list;
    coap_context_t *context = coap_session_get_context(session);//explained: contexto creado por el proxy al inicializarse
...
//explained: función enargada de manejar las peticiones entrantes del proxy
static void
hnd_proxy_uri(coap_resource_t *resource COAP_UNUSED,
    ...
    coap_opt_iterator_t opt_iter;
    coap_opt_t *opt;
    coap_opt_t *proxy_uri;//explained: opción requerida o el proxy-scheme
...
#define BUFSIZE 100
    ...
    int res;
    coap_bin_const_t token = coap_pdu_get_token(request);
    //explained: comprueba si proxy-uri o proxy-scheme
    ...
    //explained: condicional que distingue si el esquema de la URI es COAP o HTTP
    //explainedCOAP: realiza función de proxy tradicional, envía directamente la petición a la dirección indicada
    ...
    if (uri.scheme == COAP_URI_SCHEME_COAP ||
        ...
        //explainedHTTP: se mapea la petición a HTTP con libcurl (se tiene easy o curl_easy como interfaz)
        //explained: se inicializan recursos, variables necesarias, se reservan espacios de memoria
        ...
        if(curl) {
            ...
            //explained: se obtiene el host de la dirección y se junta pa tener la URI completa
            /* Si hay path*/
            ...
            //explained: se obtiene el método requerido por la petición para configurarlo en el msg http
            /* Metodo de la peticion */
            char *method;
            char *body;
            switch (coap_pdu_get_code(request)) { //se le pasa como argumento el PDU de la petición
            //explained: se almacena el método y para put y post se extraen los datos del body
                case 1:
                    method = "GET";
                    break;
                case 2:
                    ...
                    if (coap_get_data_large(request, &size, &data, &offset, &total)) {
                        ...
                        memcpy(body_data->s, data, size);
                        data = body_data->s;
                        curl_easy_setopt(curl, CURLOPT_POSTFIELDSIZE, size); //configura el tamaño del body
                        curl_easy_setopt(curl, CURLOPT_POSTFIELDS, data); //configura los datos del body
            ...
            /*explained: Las opciones se extraen mediante la función coap_check_option, a la que se le
            especifica la opción que se quiere obtener, y en caso de que venga informada, devuelve
            un puntero a su dirección de memoria. Del puntero se extrae el valor mediante la función
            coap_opt_value a la que se le pasa el puntero a la opción. */
            ...
            //explained: obtenidos los campos necesarios, se mapean pasando la opc a configurar con el vlr deseado

            curl_easy_setopt(curl, CURLOPT_HTTPHEADER, optionsToHeaders); //se asigna lista de opciones CURL en que se guardaron los valores extraídos
            curl_easy_setopt(curl, CURLOPT_CUSTOMREQUEST, method); //configura el método de la petición
            /* send all data to this function  */
            curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, cb); //establece una función de callback que será llamada en cuando reciba una respuesta para manejarla


            /* we pass our 'chunk' struct to the callback function */
            curl_easy_setopt(curl, CURLOPT_WRITEDATA, (void *) &chunk); //se le asigna un puntero que será pasado a la función de callback

            curl_easy_setopt(curl, CURLOPT_URL, host2); //configura la URL de la petición HTTP que será enviada

            //explained: envía la petición HTTP mapeada (devuelve cero si fue correcto)
            ...
            if (res != CURLE_OK) {
                ...
                //guarda la respuesta del servidor destino
                curl_easy_getinfo(curl, CURLINFO_RESPONSE_CODE, &response_code);
                //las cabeceras extrayendo las opciones, especificando en la opción el nombre de la cabecera
                res = curl_easy_getinfo(curl, CURLINFO_CONTENT_TYPE, &contentType);
                if(contentType != NULL){
                    cmdline_content_type(contentType, COAP_OPTION_CONTENT_FORMAT);
                    //recibe el PDU de respuesta donde va a mapear las opc y la lista de todas las opc a mapear
            ...
            //explained: enviando la respuesta del servidor HTTP al cliente CoAP

            switch (response_code) {//la condición es el código HTTP de respuesta
                case 200://cada case mapea el equivalente a código CoAP
                    if (strcmp(method, "DELETE") == 0) {
                        //función que mapea el código de respuesta
                ...
                default://en caso que el código de respuesta HTTP no tenga equivalente en CoAP
                    coap_pdu_set_code(response, COAP_RESPONSE_CODE_INTERNAL_ERROR);
            }
            ...
            if (response_code == 200 || response_code == 201 || response_code == 202 || response_code == 203 ||
                ...
                //explained: variable que contiene el body de la respuesta
                coap_add_data_large_response(resource, session, request, response,
                                             query, COAP_MEDIATYPE_TEXT_PLAIN, 1, 0,
                                             chunk.size,
...
#endif /* SERVER_CAN_PROXY */
//explained: Inicialización de distintos recursos
static void
init_resources(coap_context_t *ctx) {
    coap_resource_t *r;
    //explained: asigna a un recurso distintos manejadores (handlers)
    ...
    my_clock_base = clock_offset;
    //explained: estos handler se asignan en función del tipo de método que indique la petición entrante
    ...
    coap_add_resource(ctx, r);
    time_resource = r;
    //explained: sección condicional dependiente de que el servidor este configurado para actuar como proxy
...
//explained: sección condicional dependiente de que el servidor este configurado para actuar como proxy
    ...
    if (proxy_host_name_count) {
        //recurso encargado de manejar las peticiones que lleguen cuando el server está en función de proxy
...
main(int argc, char **argv) {
    ...
    //explained: ¿Descriptores de fichero?¿Bucle While principal?
    ...
    //explained: ¿Bucle While principal?
    while (!quit) {
        int result;
        ...
        if (coap_fd != -1) {
            ...
            result = select(nfds, &readfds, NULL, NULL, &tv); //explained: select() que ¿monitoriza los descriptores?
            if (result == -1) {
                if (errno != EAGAIN) {
            ...
            if (result > 0) { //explained: caso descriptor de fichero con una conexión entrante
    ...
    free(proxy_list); //explained: el método main libera todos los espacios de memoria reservados
    proxy_list = NULL;
    proxy_list_count = 0;
#ifdef _WIN32
